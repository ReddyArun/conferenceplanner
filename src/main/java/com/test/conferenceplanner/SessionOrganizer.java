...
/**
 * Class to arranges the Talk objects into Session objects
 * 
 * @author Arun Kumar
 *
 */
public class SessionOrganizer {
	...
	/**
	 * Arranges the Talk objects into Session objects by giving them a starting
	 * time. Sessions are then arranged into Track objects. Then it generates an
	 * output file containing the schedule.
	 * 
	 * @param inputFileName
	...
	public SessionOrganizer(String inputFileName) throws FileFormatException, IOException {
		logger.info("Conference Planner Program Started!");
		// Create an inputParser object whose sole purpose is to parse input
		// file into an array of talks.
		ArrayList<Talk> talks = ParserInput.parseFile(inputFileName);

		this.tracks = generateTracksFromTalk(talks);

		// Create a fileOutputProducer object whose sole purpose is to produce
		// output file from tracks.
		...
		logger.info("Conference Planner Program Just Completed!");
	}
	...
	private ArrayList<Track> generateTracksFromTalk(ArrayList<Talk> talks) {
		...
		// Sort talks based on duration, the talk with the longest duration
		// first.
		...
		for (int z = 0; z < numberOfTracks; z++) {
			...
			// Arrange afternoon sessions that run from 1pm to between 4pm and
			// 5pm.
	...
	private void arrangeAfternoonSessions(ArrayList<Talk> talks, Track track) {
		...
		Date afternoonSessionTime = DateTimeFormat.generateTime(13, 0);
		int afternoonDurationCredit = 240;
		while ((afternoonDurationCredit > 0) && (talks.isEmpty() == false)) {

			// Look for the next talk that fits into the remaining duration and
			// create a session by giving it a start time.
			for (Iterator<Talk> i = talks.iterator(); i.hasNext();) {
				...
				if (t.getMinuteDuration() <= afternoonDurationCredit) {
					...
					afternoonSessionTime = DateTimeFormat.addMinutesToTime(afternoonSessionTime, t.getMinuteDuration());
					track.addSession(s);
					talks.remove(t);
					break;
				}
	...
	private void insertLunch(Track track) {
		...
		Session lunchSession = new Session(lunchTalk, DateTimeFormat.generateTime(12, 0));
	...
	private void arrangeMorningSessions(ArrayList<Talk> talks, Track track) {
		...
		Date morningSessionTime = DateTimeFormat.generateTime(9, 0);
		...
		while ((morningDurationCredit > 0) && (talks.isEmpty() == false)) {
			...
			for (Iterator<Talk> i = talks.iterator(); i.hasNext();) {
				...
				if (t.getMinuteDuration() <= morningDurationCredit) {
					...
					morningSessionTime = DateTimeFormat.addMinutesToTime(morningSessionTime, t.getMinuteDuration());
					track.addSession(s);
					talks.remove(t);
					break;
				}
